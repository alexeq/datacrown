...
public class SoftpediaSearch extends SearchTask {
    ...
    private void setScreenshots(DcObject dco, Document document) throws Exception {
        ...
        if (node != null) {
            ...
            Document imagesDoc = HtmlUtils.getDocument(new URL(link), "UTF-8", 0);
            
            expression = "//img[contains(@src,'/screenshots/')]/@src";
    ...
    private void setCategory(DcObject dco, Document document) throws Exception {
        Node node = (Node) xpath.evaluate("//a[@class='active_category']", document, XPathConstants.NODE);
        if (node != null)
        	dco.createReference(Software._K_CATEGORIES, StringUtils.trim(node.getTextContent()));
    }
    ...
    protected Collection<Object> getItemKeys() throws Exception {
        ...
        NodeList nodeList = (NodeList) xpath.evaluate("//div[@class='skres_notice']/following::table//a[starts-with(@href, '" + mode.getKeyBaseURL()+ "')]/@href", document, XPathConstants.NODESET);
        ...
        nodeList = (NodeList) xpath.evaluate("//table[@class='narrow_listheadings']//a[starts-with(@href, '" + mode.getKeyBaseURL()+ "')]/@href", document, XPathConstants.NODESET);
        for(int i = 0; i < nodeList.getLength(); i++) {
            String link = nodeList.item(i).getTextContent();            
            if (!keys.contains(link) && link.toLowerCase().indexOf("/trailer/") == -1)
                keys.add(link);
