 ...
 */
public final class HttpOAuthHelper {

	private static Logger logger = Logger.getLogger(HttpOAuthHelper.class.getName());
    ...
    public HttpOAuthHelper(String headerAccept) {
     ...
     */
    public void handleRequest(final HttpRequest request, final String accessKeyId, final String secretAccessKey) {
     ...
     */
    public void handleRequest(final HttpRequest request, final String accessKeyId, final String secretAccessKey, final String sessionId) {
     ...
     */
    public void handleRequest(final HttpRequest request, final String accessKeyId, final String secretAccessKey, final String sessionId, final List<NameValuePair> httpParameters) {
     ...
     * @param httpParameters The HTTP parameters.
     */
    public void handleRequest(final HttpRequest request, final String accessKeyId, final String secretAccessKey, final String sessionId, final String body, final List<NameValuePair> httpParameters) {
        // Session-ID (optional)
        if (sessionId != null) {
        ...
        if (request.getRequestLine() != null && request.getRequestLine().getMethod() != null && request.getRequestLine().getMethod().equalsIgnoreCase("POST")) {
            request.addHeader(HEADER_CONTENT_TYPE, "x-www-form-urlencoded");
        }

        // Accept
        request.addHeader(HEADER_ACCEPT, headerAccept);
        ...
        if (body != null) {
            ...
            try {
            	...
            	logger.error("An error occured while handling the request: " + request, e);
            }

            if (entityAsBytes != null) {
                request.addHeader(HEADER_CONTENT_MD5, new String(Base64.encodeBase64(DigestUtils.md5(entityAsBytes))));
            }
        }

        // Date
        final DateTime dt = new org.joda.time.DateTime();
        request.addHeader(HEADER_DATE, DATE_TIME_FORMATTER.print(dt));

        // Authorization
        try {
            final String stringToSign = createStringToSign(request, httpParameters);
        	...
        	logger.error("The key is not available", e);
     ...
     */
    public String createStringToSign(final HttpRequest request, final List<NameValuePair> httpParameters) throws URISyntaxException, UnsupportedEncodingException {
     ...
     */
    public String calculateHMAC256(final String stringToSign, final String secretAccessKey) throws NoSuchAlgorithmException, InvalidKeyException {
